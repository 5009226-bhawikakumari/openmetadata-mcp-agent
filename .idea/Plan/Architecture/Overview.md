...
| **Agent Orchestrator**     | LangGraph + `langchain-openai`                       | NL understanding, intent → tool selection, multi-step workflows. (GitHub MCP is mocked internally for the Phase 3 demo; OM uses `data-ai-sdk[langchain]` natively.) |
